...
class CustomDataLoader:
    ...
    def process(self, annotation):
        ...
        if self.transform:
            ...
            for i in range(len(bboxes)): # TODO: check reassign augmented formatted box and class id
                object_annotations[2:6] = self.format_yolo(bboxes[i], self.image_size)
                object_annotations[1]   = category_ids[i]
        else:
            for i in range(len(bboxes)): # TODO: check reassign augmented formatted box and class id
                object_annotations[2:6] = self.format_yolo(bboxes[i], self.image_size)
    ...
    def create_masks(self, annotation , CreateMasks = True ) :
        ...
        for label_info in annotation['labels']:
            ...
            if category in self.class_mapping.keys():
                ...
                # x1 = int(label_info['box2d']['x1'])/1280
                # y1 = int(label_info['box2d']['y1'])/720
                # x2 = int(label_info['box2d']['x2'])/1280
                # y2 = int(label_info['box2d']['y2'])/720

                # return box without normalization 
    ...
    def format_yolo(self, box, image_size):
        xc, yc, wb, hb = box[0]/(image_size[1]), box[1]/image_size[0], box[2]/image_size[1], box[3]/image_size[0]
        return xc, yc, wb, hb
