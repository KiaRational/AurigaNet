...
class CustomDataLoader:
    def __init__(self, data_path, label_path, image_size=(720, 1280), normalize=True, class_mapping=None , train=True , transform=True):
        ...
        if self.transform:
            ...
            self.transforms = A.Compose(transforms=[
                                                ...
                                                A.MotionBlur(blur_limit=17, p=0.05),
                                                A.ImageCompression(quality_lower=39, quality_upper=60, p=0.1),
                                                A.HorizontalFlip(p=0.2),
                                                A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.075, rotate_limit=20, p=0.1,border_mode=cv2.BORDER_CONSTANT)
    ...
    def process(self, annotation):
        ...
        lane_clustered_mask = self.resize_mask(lane_clustered_mask,(640,640))

        lane_clustered_mask = self.max_pooling_2d(self.max_pooling_2d(lane_clustered_mask))
        # print(lane_clustered_mask.shape)
        ...
        drivable_clustered_mask = self.resize_mask(drivable_clustered_mask,(160,160))
        ...
        # drivable_clustered_mask = self.resize_mask(drivable_clustered_mask,(640,640))
    ...
    def create_masks(self, annotation , CreateMasks = True , train=True) :
        ...
        for label_info in annotation['labels']:
            ...
            if category in self.class_mapping.keys() or category == 'truck' or category == 'bus' or category == 'train':

                class_index = 0 # self.class_mapping[category]
