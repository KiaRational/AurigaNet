...
class EmbeddingLoss(nn.Module):
    ...
    def forward(self, x, cluster_masks):
        ...
        return loss / batch_size , intra_cluster_loss / batch_size , inter_cluster_loss / batch_size
...
class DiscriminativeLoss(_Loss):
    ...
    def _discriminative_loss(self, embedding, seg_gt):
        ...
        var_loss = self.alpha * var_loss / batch_size
        dist_loss = self.beta * dist_loss / batch_size
        reg_loss = self.gamma * reg_loss / batch_size
        if torch.isnan(var_loss):
            var_loss = torch.zeros_like(var_loss)
        if torch.isnan(dist_loss):
            var_loss = torch.zeros_like(dist_loss)
        if torch.isnan(reg_loss):
            var_loss = torch.zeros_like(reg_loss)
...
def multiclass_dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon: float = 1e-6):
    ...
    return dice_coeff(input.flatten(0, 1), target.flatten(0, 1), reduce_batch_first, epsilon)#/(input.shape[0])
...
#         self.Feature_Embedding_Loss =DiscriminativeLoss(0.5, 1.5,2.0, 1.0, 1.0, 0.001) # EmbeddingLoss(self.P.embedding_alpha,self.P.embedding_delta,self.P.embedding_lambda_inter,self.P.embedding_lambda_intra,'euclidean') ### 
...
#         if not train:
#             return None, (None,None,None) , [t1,t2,t3]
...
#         SegLoss_da = self.Segmentation_Confidence_Loss(Pred_Confidence[:,0,:,:],GroundTruth_Confidence[:,0,:,:])
        
#         dice_da = dice_loss(activation(Pred_Confidence[:,0,:,:]),GroundTruth_Confidence[:,0,:,:],multiclass=False)

#         SegLoss_ll = self.Segmentation_Confidence_Loss(Pred_Confidence[:,1,:,:],GroundTruth_Confidence[:,1,:,:])
        
#         dice_ll = dice_loss(activation(Pred_Confidence[:,1,:,:]),GroundTruth_Confidence[:,1,:,:],multiclass=False)
#         # time1 = time.time()
#         AreaFeatureLoss = self.Feature_Embedding_Loss(Pred_EmbeddingFeatureArea,GroundTruth_EmbeddingFeatureArea)
#         AreaFeatureLoss_total =  self.P.Alpha3 * (AreaFeatureLoss[0] +  AreaFeatureLoss[1] +  AreaFeatureLoss[2] )
#         if torch.isnan(dice_da):
#             dice_da = torch.zeros_like(dice_da)
#         if torch.isnan(dice_ll):
#             dicd_ll = torch.zeros_like(dice_ll)
#         if torch.isnan(AreaFeatureLoss_total):
#             if torch.isnan(self.P.Alpha3 * AreaFeatureLoss[2]):
#                 AreaFeatureLoss_total = self.P.Alpha3 *( AreaFeatureLoss[0] +  AreaFeatureLoss[1])
#             if torch.isnan(self.P.Alpha3 * AreaFeatureLoss[0]) or torch.isnan(self.P.Alpha3 * AreaFeatureLoss[1]):
#                 AreaFeatureLoss_total = torch.zeros_like(AreaFeatureLoss_total)
#         TotalLoss = self.P.Alpha1*(SegLoss_da+SegLoss_ll) + self.P.Alpha2 * (dice_da+dice_ll) + self.P.Alpha4 * ObjLoss + AreaFeatureLoss_total
#         # print(TotalLoss,SegLoss.item(),dice.item(),ObjLoss.item(),AreaFeatureLoss[0].item())
#         if train:
#             return TotalLoss ,(objdetail[0].item(),objdetail[1].item(),objdetail[2].item(),(self.P.Alpha1*(SegLoss_da+SegLoss_ll) + self.P.Alpha2 * (dice_da+dice_ll)).item() , ( self.P.Alpha3 * AreaFeatureLoss[0] + self.P.Alpha3/10 * AreaFeatureLoss[1] + self.P.Alpha3/1000 * AreaFeatureLoss[2] ).item())
#         else:
#             return TotalLoss , (objdetail[0].item(),objdetail[1].item(),objdetail[2].item(),(self.P.Alpha1*(SegLoss_da+SegLoss_ll) + self.P.Alpha2 * (dice_da+dice_ll)).item() , ( self.P.Alpha3 * AreaFeatureLoss[0] + self.P.Alpha3/10 * AreaFeatureLoss[1] + self.P.Alpha3/1000 * AreaFeatureLoss[2] ).item()) , [t1,t2,t3]
...
#         # Indexing along a single dimension
#         pxy_obj = pxy[obj]
#         pwh_obj = pwh[obj]

#         pbox = torch.cat((pxy_obj, pwh_obj), dim=-1)
...
#              + self.lambda_class * lcls) *bs/3 ,

#             [
#             self.lambda_box * lbox *bs/3,
#             self.lambda_obj * lobj *bs/3,
#             self.lambda_class * lcls *bs/3
#             ]
...
class ComputeLoss:
    ...
    def __init__(self, device, autobalance=False):
        ...
        self.lambda_class = 0.5 * (self.nc / 80 * 3 / self.nl)
        self.lambda_obj = 2 * ((h.image_size / 640) ** 2 * 3 / self.nl) # was 1 at first before
        self.lambda_box = 0.05 * (3 / self.nl)
    ...
    def __call__(self, predictions, targetss, pred_size, train=True):
        preds = predictions[2][0]
        targets = targetss[2]
        targets = [self.build_targets(preds, bboxes[...,1:], pred_size) for bboxes in targets]
    ...
    def build_targets(self, input_tensor, bboxes, pred_size):
        ...
        if check_loss:
            ...
            targets = [torch.zeros((self.num_anchors_per_scale, int(pred_size[0] / S), int(pred_size[1] / S), 6))
                       for S in self.S]
    ...
    def compute_loss_det(self, preds, targets, anchors, balance):
        ...
        iou = intersection_over_union(pbox, tbox, GIoU=False,SIOU=True).squeeze()
        if has_obj > 0:
            lbox = (1.0 - iou).mean()  # IoU loss
        else:
            lbox = torch.tensor(0)

        iou = iou.detach().clamp(min=0.1)  # Clamping IoU to avoid zeroing out the gradient
        targets[..., 4][obj] *= iou

        lobj_bce = self.focal_loss(preds[..., 4], targets[..., 4])  # Focal Loss for objectness
        lobj_iou = self.BCE_obj(preds[..., 4], targets[..., 4]) * balance  # BCE Loss for objectness
        lobj = lobj_bce + lobj_iou  # Summing Focal Loss and IoU Loss

        if has_obj > 0:
            tcls = torch.zeros_like(preds[..., 5:][obj], device=self.device)
            tcls[torch.arange(tcls.size(0)), targets[..., 5][obj].long()] = 1.0
            lcls = self.focal_loss(preds[..., 5:][obj], tcls)  # Focal Loss for classification
        else:
            lcls = torch.tensor(0)
        ...
        return (
            ...
            [
                self.lambda_box * lbox * bs,
                self.lambda_obj * lobj * bs,
                self.lambda_class * lcls * bs
            ]
        )
...
class ComputeLoss_Ultra:
    ...
    def __call__(self, predictions, targetss , pred_size):  # predictions, targets
        p = predictions[2][0]
        targets = targetss[2]
        ...
        TotalLoss = self.P.Alpha1*(SegLoss_da+SegLoss_ll) + self.P.Alpha2 * (dice_da+dice_ll) + self.P.Alpha4 * ObjLoss + AreaFeatureLoss_total
        # print(TotalLoss,SegLoss.item(),dice.item(),ObjLoss.item(),AreaFeatureLoss[0].item())
        return TotalLoss ,((lbox*4).item(),(lobj*4).item(),(lcls*4).item(),(self.P.Alpha1*(SegLoss_da+SegLoss_ll) + self.P.Alpha2 * (dice_da+dice_ll)).item() , ( self.P.Alpha3 * AreaFeatureLoss[0] + self.P.Alpha3/10 * AreaFeatureLoss[1] + self.P.Alpha3/1000 * AreaFeatureLoss[2] ).item())
        
        # return TotalLoss, torch.cat((lbox*4, lobj*4, lcls*4)).detach()
    ...
    def build_targets(self, p, targets):
        ...
        targets = torch.cat((targets.repeat(na, 1, 1).to(self.device), ai[..., None]), 2)  # append anchor indices
...
class TargetBuilder:
    ...
    def __call__(self, predictions, targetss , pred_size , train=True):  # predictions, targets
        preds = None#predictions[2][0]
        targets = targetss#.unsqueeze(0)
