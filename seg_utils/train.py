...
def train_step(model,dataloader,loss_fn,accuracy_fn,optimizer,grad_scaler,device,epoch,num_warmup,Parameters,lf,SegOnly=False):
    ...
    for i, (images, confidence_mask, cluster_masks , object_annotations ) in enumerate(tqdm(dataloader)):
        ...
        if SegOnly:
            ...
            targets = [confidence_mask.to(device),instance_drivable.to(device),object_annotations]
...
def evaluate(net, dataloader, device , save_path , Parameters , loss_fn):
    ...
    with torch.autocast(device):
        ...
        for i, (images, confidence_mask, cluster_masks , object_annotations) in enumerate(tqdm(dataloader)):
            ...
            targ = [confidence_mask.to(device),instance_drivable.to(device),object_annotations]
            ...
            # T50 , T75 , obj_acc , class_acc , confusion_matrix = mAP(object_annotations , pred_boxes)
            # TPFP50 += T50 
            # TPFP75 += T75
    ...
    # mAP50 = TPFP50[:,1]/(TPFP50[:,0]+TPFP50[:,1]+1e-16)
    # mAP75 = TPFP75[:,1]/(TPFP75[:,0]+TPFP75[:,1]+1e-16)
