...
class CustomDataLoader:
    ...
    def load_data(self):
        json_file_path = os.path.join(
            self.label_path, 'bdd100k_labels_images_train.json')
    ...
    def process_batch(self, batch_annotations):
        ...
        for annotation in batch_annotations:
            ...
            if os.path.exists(image_path):
                ...
                drivable_mask, lane_mark_mask, obj_annot = self.create_masks(
                    image, annotation)
                ...
                # Add a new axis to make it (1, H, W)
                batch_drivable_masks.append(drivable_mask[np.newaxis])
                # Add a new axis to make it (1, H, W)
                batch_lane_masks.append(lane_mark_mask[np.newaxis])

                batch_obj_annots.append(obj_annot)

            else:
                continue

            return batch_images, batch_drivable_masks, batch_lane_masks, batch_obj_annots

    def create_masks(self, image, annotation):
        lane_masks = np.zeros_like(image[:, :, 0])
        drivable_area_masks = np.zeros_like(image[:, :, 0])
        obj_annot = np.zeros((1, 5))
        ...
        for label_info in annotation['labels']:
            ...
            if category in class_mapping.keys():
                ...
                xc, yc, wb, hb = self.format_yolo(
                    [box_center_x, box_center_y, box_width, box_height], image)
        ...
        return drivable_area_masks, lane_masks, obj_annot

    def format_yolo(self, box, image):
        xc, yc, wb, hb = box[0]/image.shape[1], box[1] / \
            image.shape[0], box[2]/image.shape[1], box[3]/image.shape[0]
        return xc, yc, wb, hb
...
class CustomDataset(Dataset):
    def __init__(self, data_path, label_path, image_size=(720, 1280), normalize=True, class_mapping=None, batch_size=8):
        self.data_loader = CustomDataLoader(
            data_path, label_path, image_size, normalize, class_mapping, batch_size)
        self.batch_size = batch_size
        self.image_size = image_size
    ...
    def __getitem__(self, index):
        ...
        end_idx = min((index + 1) * self.batch_size,
                      self.data_loader.num_samples)
        batch_annotations = self.data_loader.annotations[start_idx:end_idx]

        batch_images, batch_drivable_masks, batch_lane_masks = self.data_loader.process_batch(
            batch_annotations)

        if len(batch_images) == 0:  # Handle empty batch
            batch_images = np.zeros((1, *self.image_size, 3), dtype=np.float32)
            batch_drivable_masks = np.zeros(
                (1, *self.image_size), dtype=np.float32)
            batch_lane_masks = np.zeros(
                (1, *self.image_size), dtype=np.float32)
        ...
        batch_images = torch.tensor(
            batch_images.transpose(0, 3, 1, 2), dtype=torch.float32)
        batch_drivable_masks = torch.tensor(
            batch_drivable_masks, dtype=torch.float32)  # No need for unsqueeze
        batch_lane_masks = torch.tensor(
            batch_lane_masks, dtype=torch.float32)  # No need for unsqueeze
